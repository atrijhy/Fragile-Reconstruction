...
def get_network(arch: str, isTrain=False, continue_train=False, init_gain=0.02, pretrained=True, cfg=None):
    if "resnet" in arch:
        if arch.startswith("dire_resnet"):
            from networks.dire_resnet import DireOdeResNet

            base_arch = arch.replace("dire_", "")
            return DireOdeResNet(base_arch, cfg, is_train=isTrain, continue_train=continue_train, init_gain=init_gain, pretrained=pretrained)
        ...
        if isTrain:
            if continue_train:
                ...
                # set classifier output to single logit, but respect base fc in_features
                in_features = getattr(getattr(model, 'fc', None), 'in_features', None)
                if in_features is None:
                    # fallback to resnet50 default
                    in_features = 2048
