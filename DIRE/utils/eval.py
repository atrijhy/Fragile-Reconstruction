...
def validate(model: nn.Module, cfg: CONFIGCLASS, preprocess_fn=None):
    from sklearn.metrics import accuracy_score, average_precision_score, roc_auc_score

    from utils.datasets import create_dataloader

    # helpful runtime hint: show which dirs are used when dire_mode
    if getattr(cfg, "dire_mode", False):
        val_real = getattr(cfg, "dire_val_real", "")
        val_fake = getattr(cfg, "dire_val_fake", "")
        use_kfold = bool(getattr(cfg, "dire_use_kfold_val", False))
        if val_real and val_fake:
            print(f"[validate] Using validation dirs (real/fake): {val_real} | {val_fake}")
        elif use_kfold:
            print(
                f"[validate] Using K-fold validation from training dirs: k={getattr(cfg,'dire_kfold_k',5)}, fold_idx={getattr(cfg,'dire_kfold_idx',0)}"
            )
        else:
            print(f"[validate] Using TEST dirs (real/fake): {getattr(cfg,'dire_test_real','')} | {getattr(cfg,'dire_test_fake','')}")
