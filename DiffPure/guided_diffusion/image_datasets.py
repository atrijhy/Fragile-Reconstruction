...
def load_data_for_reverse(
    *,
    ...
    """
    Guided-diffusion loader variant used when running reverse ODE/AutoAttack.
    Yields tensors in [-1,1] along with metadata dict and original path.
    """
    if not data_dir:
    ...
    if MPI.COMM_WORLD.Get_rank() == 0:
        ...
        all_files = None
    all_files = MPI.COMM_WORLD.bcast(all_files)
    ...
    dataset = ImageDatasetForReverse(
        image_size,
        all_files,
        classes=classes,
...
class ImageDatasetForReverse(Dataset):
    def __init__(
        self,
        resolution,
        image_paths,
