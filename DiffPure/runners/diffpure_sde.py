...
class RevGuidedDiffusion(torch.nn.Module):
    def __init__(self, args, config, device=None):
        ...
        if config.data.dataset == 'ImageNet':
            ...
            # Prefer an explicit model path passed via args.diffusion_model_path; fall back
            # to the repo's pretrained directory if not provided.
            model_path = getattr(args, 'diffusion_model_path', None)
            if model_path is None:
                model_path = os.path.join(model_dir, '256x256_diffusion_uncond.pt')
            print(f'Loading guided-diffusion model from: {model_path}')
