...
# NOTE:
# score_sde brings in custom CUDA/C++ ops (StyleGAN2 fused_act / upfirdn2d) that JIT-compile
# at import time via torch.utils.cpp_extension.load. This can stall ImageNet workflows that
# never use score_sde. To keep ImageNet attacks/exports responsive, we import score_sde
# lazily only when needed (e.g., CIFAR10 branch or score_type == 'score_sde').
...
class VPODE(torch.nn.Module):
    ...
    def ode_fn(self, t, x):
        ...
        if self.score_type == 'guided_diffusion':
            ...
            # model output is epsilon
            # Lazy import to avoid triggering score_sde custom op compilation for ImageNet runs.
            from score_sde import sde_lib as _sde_lib
            from score_sde.models import utils as _mutils

            sde = _sde_lib.VPSDE(beta_min=self.beta_0, beta_max=self.beta_1, N=self.N)
            score_fn = _mutils.get_score_fn(sde, self.model, train=False, continuous=True)
...
class OdeGuidedDiffusion(torch.nn.Module):
    def __init__(self, args, config, device=None):
        ...
        # load model
        # default score type and step size if not provided on args (DiffPure CLI exposes these)
        if not hasattr(args, 'score_type'):
            setattr(args, 'score_type', 'guided_diffusion')
        if not hasattr(args, 'step_size'):
            setattr(args, 'step_size', 2e-2)

        if config.data.dataset == 'ImageNet':
            # Derive image shape dynamically from model config to support 64x64 and 256x256
            model_config = model_and_diffusion_defaults()
            model_config.update(vars(self.config.model))
            img_size = int(model_config.get('image_size', 256))
            ...
            if args.diffusion_model_path:
                ...
                model_path = os.path.join('pretrained', 'guided_diffusion', '256x256_diffusion_uncond.pt')
            print(f"[ODE] loading diffusion checkpoint from: {model_path}")
