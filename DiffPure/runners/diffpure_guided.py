...
class GuidedDiffusion(torch.nn.Module):
    ...
    def recon_from_x(self, x, steps=None):
        """
        Official DIRE reconstruction: encode with DDIM reverse to latent, then
        decode with DDPM sampling starting from that latent.

        Args:
            x: input image in [-1,1], shape [B,C,H,W]
            steps: number of discrete timesteps (defaults to self.args.t)
        Returns:
            reconstruction in [-1,1], same shape as x
        """
        ...
        # DDIM reverse loop expects shape and uses the provided noise=x as x0
        ...
        # DDPM forward sampling from the encoded latent
        recon = self.diffusion.p_sample_loop(
            self.model,
            x.shape,
            noise=latent,
            clip_denoised=True,
